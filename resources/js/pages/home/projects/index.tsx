...
import { ProjectsHeader, type ProjectFilters } from './partials/projects-header';
...
const defaultFilters: ProjectFilters = { types: [], creators: [] };
...
export default function Projects() {
    const { projects, types, creators } = usePage<PageProps>().props;
    ...
    const filtered = useMemo(() => {
        ...
        if (filters.creators.length)
            list = list.filter((p) => p.creator?.name && filters.creators.includes(p.creator.name));
        if (search.trim())
            list = list.filter(
                (p) =>
        ...
        if (sort === 'creator') list.sort((a, b) => (a.creator?.name ?? '').localeCompare(b.creator?.name ?? ''));
    ...
    const handleFilters = (f: ProjectFilters) => { setFilters(f); setPage(1); };
            ...
            <div className="relative w-full px-4 py-10 md:px-6 lg:px-10">
                ...
                {paginated.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center rounded-2xl border border-border/40 bg-background/60 py-20 text-center backdrop-blur-sm">
                        <p className="text-lg font-medium text-foreground/60">Aucun projet trouvé</p>
