...
const sortOptions = [
    ...
    { value: 'creator', label: 'Par créateur (A–Z)' },
];
...
type Props = {
    ...
    onFilters: (f: ProjectFilters) => void;
    total: number;
    typeOptions: string[];
    creatorOptions: string[];
};

export function ProjectsHeader({ search, onSearch, sort, onSort, filters, onFilters, total, typeOptions, creatorOptions }: Props) {
    const [sortOpen, setSortOpen] = useState(false);
    const current = sortOptions.find((o) => o.value === sort);

    const activeCount = filters.types.length + filters.creators.length;
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-10">
            ...
            <div className="mb-4 flex flex-col gap-3 sm:flex-row">
                <div className="relative flex-1">
                    ...
                    <Input
                        type="search"
                        placeholder="Rechercher un projet, un type, un créateur..."
                        value={search}
            ...
            <div className="flex flex-wrap items-center gap-2">
                <MultiSelect
                    ...
                    onChange={(types) => onFilters({ ...filters, types })}
                    searchable
                />
                <MultiSelect
                    label="Créateur"
                    ...
                    onChange={(creators) => onFilters({ ...filters, creators })}
                    searchable
                />
                ...
                {activeCount > 0 && (
                    <button
                        type="button"
                        onClick={() => onFilters({ types: [], creators: [] })}
