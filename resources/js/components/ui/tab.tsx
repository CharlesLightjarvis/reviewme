...
const TabsTriggerList = ({
  ...
  return (
    <BaseTabs.List className={cn("flex flex-row gap-1 w-fit rounded-lg bg-muted/50 p-1", className)} {...props}>
      {children}
    </BaseTabs.List>
  );
};
...
const TabsTrigger = ({ children, className, ...props }: ITabsTrigger) => {
  return (
    <BaseTabs.Tab
      className={cn(
        "px-4 flex items-center py-1 rounded-md border-2 border-transparent text-foreground/60 data-[active]:border-border data-[active]:bg-primary data-[active]:text-primary-foreground data-[active]:font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary",
        className,
      )}
      {...props}
    >
