...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<"ol">>(
  ({ className, ...props }, ref) => (
    <ol
      ref={ref}
      className={cn("flex flex-wrap items-center gap-1.5 sm:gap-2.5 text-muted-foreground", className)}
      {...props}
    />
  ),
);
...
>(({ asChild, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn("font-medium transition-colors hover:text-foreground focus:outline-none focus:ring-2 focus:ring-ring rounded-sm", className)}
      {...props}
    />
  );
});
