...
const DialogBackdrop = (inputProps: IDialogBackdropProps & { ref?: React.Ref<HTMLDivElement> }) => {
...
const dialogFooterVariants = cva(
  "flex items-center justify-end border-t-2 min-h-12 gap-4 px-4 py-2",
  {
    variants: {
      ...
      position: { fixed: "sticky bottom-0", static: "static" },
    },
...
const dialogHeaderVariants = cva(
  "flex items-center justify-between border-b-2 px-4 min-h-12",
  {
    variants: {
      ...
      position: { fixed: "sticky top-0", static: "static" },
    },
    defaultVariants: { variant: "default", position: "static" },
  },
);
...
const DialogTitle = ({ children, className, ...props }: HTMLAttributes<HTMLHeadingElement>) => (
  <BaseDialog.Title className={cn("font-semibold", className)} {...props}>
    {children}
  </BaseDialog.Title>
);
